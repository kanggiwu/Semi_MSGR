...
public class MessengerDAO {
	...
	/**
	 * alt + shift + j 로그인 메서드 SELECT mem_id_vc, mem_nick_vc FROM MSGR_MEMBER WHERE
	 * mem_id_vc = #{mem_id_vc} AND mem_pw_vc = #{mem_pw_vc}
	 * 
	...
	/**
	 * 회원가입의 아이디 중복확인 메서드 SELECT mem_id_vc FROM msgr_member WHERE mem_id_vc =
	 * #{mem_id_vc}
	 * 
	...
	/**
	 * 회원가입 메서드 INSERT INTO msgr_member (mem_id_vc, mem_pw_vc, mem_nick_vc) values
	...
	/**
	 * 톡방리스트 불러오기 메서드 SELECT room.room_name_vc, room.room_no_nu FROM MSGR_ROOM room,
	 * MSGR_ROOM_IN_LIST rlist WHERE room.room_no_nu = rlist.room_no_nu AND
	...
	/**
	 * 친구목록 불러오기 메서드 SELECT buddy_id_vc FROM MSGR_BUDDY WHERE mem_id_vc =
	 * #{mem_id_vd} ORDER BY mem_id_vc
	 * 
	 * @param pMap
	 * @return
	 */
	...
	/**
	 * 마지막 대화번호 가져오기 메소드 SELECT chat_no_nu FROM(SELECT chat_no_nu) FROM MSGR_CHAT
	...
	/**
	 * 톡방 참가한 이후의 대화내용 가져오기 메소드 SELECT chat.chat_vc FROM MSGR_CHAT chat,
	 * MSGR_ROOM_IN_LIST rlist WHERE chat.chat_no_nu > (SELECT join_chat_no_nu FROM
	 * MSGR_ROOM_IN_LIST WHERE mem_id_vc=#{mem_id_vc}) AND rlist.mem_id_vc =
	 * #{rlist.mem_id_vc} AND chat.roon_no_nu = #{chat.room_no_nu}
	 * 
	 * @param pMap
	 * @return
	 */
	...
	/**
	 * 회원탈퇴 메소드 { CALL BEGIN UPDATE MSGR_CHAT SET mem_id_vc = null WHERE mem_id_vc =
	 * #{mem_id_vd}; DELETE FROM MSGR_ROOM_IN_LIST WHERE mem_id_vc = #{mem_id_vc};
	...
	/**
	 * 친구삭제 메소드 { CALL BEGIN DELETE FROM MSGR_BUDDY WHERE buddy_id_vc =
	 * #{buddy_id_vc} AND mem_id_vc = #{mem_id_vc}; DELETE FROM MSGR_BUDDY WHERE
	 * buddy_id_vc = #{buddy_id_vc} AND mem_id_vc = #{mem_id_vc}; END }
	 * 
	 * @param pMap
	 * @return
	 */
	 ...
	 * 친구추가 메소드
	 * 
	 * INSERT INTO MSGR_BUDDY(buddy_seq_nu, mem_id_vc, buddy_id_vc) VALUES
	 * (buddy_seq_nu.NEXTVAL, #{msgr_mem_id), #{msgr_buddy_id};
	 * 
	 * @param pMap
	 * @return
	 */
